...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    QString name = "/home/cty/workspace/OPENGL/OpenGL_Shader/opengl_shaderLesson12/build/Terrain.bmp";
...
void QGLESWIDGET::render()
{
    ...
    _shaderM.begin();
    {
        glUniformMatrix4fv(_shaderM._MVP, 1, 0, mat2D.data());
        glUniform1i(_shaderM._texture, 0);
        glUniform2f(_shaderM._size, _width,_height);
        glEnableVertexAttribArray(_shaderM._position);
        glVertexAttribPointer(_shaderM._position,   3, GL_FLOAT, GL_FALSE, sizeof(VertexPUV), quad);
